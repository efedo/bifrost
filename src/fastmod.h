...
// MSVC has no native unsigned 128-bit integer type. Keep the same API and
// fall back to hardware integer remainder for 64-bit divisors.
using fastmod_u64_t = uint64_t;

FASTMOD_API fastmod_u64_t computeM_u64(uint64_t d) {
  return d;
}

FASTMOD_API uint64_t fastmod_u64(uint64_t a, fastmod_u64_t M, uint64_t d) {
  (void)M;
  return a % d;
}

FASTMOD_API uint64_t fastdiv_u64(uint64_t a, fastmod_u64_t M) {
  return a / M;
}

#else

using fastmod_u64_t = __uint128_t;
